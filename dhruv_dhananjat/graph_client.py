...
class GraphClient:
    ...
    def add_relationship(self, from_id: str, to_id: str, rel_type: str):
        with self.driver.session() as session:
            check = session.run(
                "MATCH (a {id: $from_id}), (b {id: $to_id}) RETURN a, b",
                from_id=from_id,
                to_id=to_id
            )
            
            if not check.single():
                print(f"   ⚠ WARNING: Could not find both entities: {from_id}, {to_id}")
                return
    ...
    def get_entity_relationships(self, entity_id: str) -> List[Dict[str, Any]]:
        with self.driver.session() as session:
            ...
            print(f"   DEBUG: Found {len(relationships)} relationships in DB for {entity_id}")
